...
export default {
  ...
  created() {
    // 获取分组信息
    this.getData("broadcastTaskGroupScreen", "deviceList", {
      taskId: this.taskId,
    ...
    //获取当前信息
    if (this.taskId) {
      this.getData("broadcastTaskQuery", "", this.taskId);
    }
  },
  methods: {
    ...
    async getData(type, val, data) {
      ...
      if (_.isEmpty(res) || res.code != "200" || res.code != 200) {
        ...
        if (type === "getMediaList") {
          ...
          this.$router.push("/application-manage/broad-cast/list?tabKey=5");
        } else if (type === "broadcastTaskQuery") {
          let list = res.data,
            tmp = [],
            gtourpTmp = [],
            valTmp = [];
        ...
        } else if (type === "broadcastTaskGroupScreen") {
          this[val] = res.data;
        } else {
          this[val] = res.data;
        }
